...
import com.festina.gameserver.Connection;
...
import com.festina.gameserver.ai.L2PlayerAI;
import com.festina.gameserver.cache.HtmCache;
import com.festina.gameserver.communitybbs.BB.Forum;
...
import com.festina.gameserver.instancemanager.FishingZoneManager;
...
import com.festina.gameserver.model.actor.knownlist.PcKnownList;
import com.festina.gameserver.model.actor.stat.PcStat;
import com.festina.gameserver.model.actor.status.PcStatus;
...
import com.festina.gameserver.model.base.SubClass;
...
import com.festina.gameserver.serverpackets.ChangeWaitType;
import com.festina.gameserver.serverpackets.CharInfo;
import com.festina.gameserver.serverpackets.ConfirmDlg;
import com.festina.gameserver.serverpackets.ExFishingEnd;
import com.festina.gameserver.serverpackets.ExFishingStart;
import com.festina.gameserver.serverpackets.ExOlympiadMode;
import com.festina.gameserver.serverpackets.ExOlympiadUserInfo;
import com.festina.gameserver.serverpackets.ExOlympiadUserInfoSpectator;
...
import com.festina.gameserver.serverpackets.MagicSkillCanceld;
import com.festina.gameserver.serverpackets.MyTargetSelected;
import com.festina.gameserver.serverpackets.NpcHtmlMessage;
import com.festina.gameserver.serverpackets.ObservationMode;
import com.festina.gameserver.serverpackets.ObservationReturn;
import com.festina.gameserver.serverpackets.PartySmallWindowUpdate;
import com.festina.gameserver.serverpackets.PetInventoryUpdate;
import com.festina.gameserver.serverpackets.PrivateStoreListBuy;
import com.festina.gameserver.serverpackets.PrivateStoreListSell;
import com.festina.gameserver.serverpackets.RecipeShopSellList;
import com.festina.gameserver.serverpackets.RelationChanged;
import com.festina.gameserver.serverpackets.Ride;
import com.festina.gameserver.serverpackets.SendTradeDone;
...
import com.festina.gameserver.serverpackets.Snoop;
...
import com.festina.gameserver.serverpackets.TargetSelected;
import com.festina.gameserver.serverpackets.TradeStart;
...
import com.festina.gameserver.templates.L2ArmorType;
import com.festina.gameserver.templates.L2Henna;
import com.festina.gameserver.templates.L2Item;
import com.festina.gameserver.templates.L2PcTemplate;
...
public final class L2PcInstance extends L2PlayableInstance
{
	...
	/** Осадочный флаг (В 16hex: 0х180 - меч, 0x80 - щит, 0xC0 - корона, 0x1C0 - флаг)*/
	...
	public static int _reviveRequested = 0;
	public static int _revivePower = 0;
...
//	public boolean isInCraftMode() { return (getPrivateStoreType() == STORE_PRIVATE_MANUFACTURE); }
	...
	public void rewardSkills()
	{
		...
		if ((getClan() != null) && (isClanLeader()))
		{
			if (getClan().getLevel() > 3)
			{
				SiegeManager.getInstance().addSiegeSkills(this);
			}
		}
			
		if (NobleTable.isNoble(getObjectId()))
		{
			NobleTable.getNobleSkill(this);
		}
		// Механизм автоизучения скилов.
	...
	public void rewardSkills1()
	  {
	    ...
	    else if (Config.DEBUG) { _log.fine("No skills awarded at lvl: " + lvl); }
	...
	*/ 
	...
	public void setProtection(boolean protect)
	{
		if (Config.DEVELOPER && (protect || _protectEndTime > 0)) 
            _log.info(this.getName() + ": Protection " + (protect?"ON " + (GameTimeController.getGameTicks() + Config.PLAYER_SPAWN_PROTECTION * GameTimeController.TICKS_PER_SECOND) :"OFF") + " (currently " + GameTimeController.getGameTicks() + ")");
...
//                      || this.getClan().isAtWarWith(((L2PcInstance)killer).getClanId())
	...
	{
	    //TODO: апдейт статус с новым зоновым движком?)
		if (
				checkIfPvP(target) &&
				(
						!this.isInsideZone(ZONE_PVP) ||       // Player is not inside pvp zone or
						!target.isInsideZone(ZONE_PVP)        // Target player is not inside pvp zone
				) &&
                (
	...
	private static L2PcInstance restore(int objectId)
	{
		...
		try
		{
			...
			while (rset.next())
			{
				...
				try
				{
					player.setBaseClass(rset.getInt("base_class")); } catch (Exception e) {
...
//			statement.setInt(30, getBaseTemplate().race.ordinal());
	...
	public boolean isAutoAttackable(L2Character attacker)
	{
		...
		if (attacker instanceof L2PcInstance)
		{
			...
			if (this.isInsideZone(ZONE_PVP) && attacker.isInsideZone(ZONE_PVP))
				return true;
	...
	public void useMagic(L2Skill skill, boolean forceUse, boolean dontMove)
	{
		...
		{
			if ((isInsidePeaceZone(this, target)) && (getAccessLevel() < Config.GM_PEACEATTACK) && (!(target instanceof L2GuardInstance)))
			{
	...
	public boolean checkPvpSkill(L2Object target, L2Skill skill)
	{
		...
		if (
				...
				!this.isInsideZone(ZONE_PVP) &&        // Pc is not in PvP zone
				!((L2Character) target).isInsideZone(ZONE_PVP)         // target is not in PvP zone
		)
		{
	...
	class WaterTask implements Runnable
	{
		...
		public void run()
		{
			 L2PcInstance._log.info("STARTED TASK");
    ...
    public void setNoble(L2PcInstance player)
    {
    	int objid = player.getObjectId();
    	if(!NobleTable.isNoble(objid))
    	{
    	NobleTable.setnoble(objid);
	...
	private void setClassTemplate(int classId)
	{
		...
		L2PcTemplate t = CharTemplateTable.getInstance().getTemplate(classId, false);
	...
	public void startWaterTask()
	{
		...
		{
			int timeinwater = (int)calcStat(Stats.BREATH, 60000.0D, this, null);
			_log.info("TRY TO START TASK: timeinwater = " + timeinwater);
...
//		if (Config.DEVELOPER) System.out.println("Move Ticks:" + (gameTicks - m._moveTimestamp) + ", distPassed:" + distPassed + ", distFraction:" + distFraction);
	...
	public void setAllianceWithVarkaKetra(int sideAndLvlOfAlliance)
	  ...
	  {
	    if ((isFlying()) || (isRiding()) || (isMounted()))
	   {
	      if (isFlying())
	  ...
	  public void reviveRequest(L2PcInstance Reviver, L2Skill skill, boolean Pet)
	  {
	    ...
	    {
	      sendMessage("Already Resurected.");
	      return;
	    }
	    if (((Pet) && (getPet() != null) && (getPet().isDead())) || ((!Pet) && (isDead())))
	    {
	      ...
	      ConfirmDlg dlg = new ConfirmDlg(1510, Reviver.getName());
	
	      sendPacket(dlg);
	    }
	  }
	
	  public void onDialogAnswer(int message, int answer)
	  {
	    if (message == 1510)
	    {
	      if (answer == 1)
	  ...
	  public void notifyFriends(boolean logout)
	  {
	    java.sql.Connection con = null;
	    try
	    {
	      ...
	      PreparedStatement statement = con.prepareStatement("SELECT char_id FROM character_friends WHERE friend_id=?");
	      statement.setInt(1, getObjectId());
	      ResultSet rset = statement.executeQuery();
	      ...
	      while (rset.next())
	      {
	        ...
	        {
	          if (!logout) {
	            friend.sendPacket(new FriendList(friend, false, this));
	          }
	          else {
	            friend.sendPacket(new FriendList(friend, true, this));
	            SystemMessage sm = new SystemMessage(503);
	...
	public void restoreProfileBuffs() {
		...
		try {
			...
			st = con.prepareStatement("SELECT profile, buffs FROM `buff_profile` WHERE `char_id` = ? ORDER BY `profile`");
	...
	public void doBuffProfile(int buffprofile) {
		...
		if (profile == null || profile.isEmpty()) {
			sendMessage("Something went wrong with buff profile: " + buffprofile);
			return;
		}
	...
	public void saveBuffProfile(int buffprofile) {
		...
		sendMessage("Profile " + buffprofile + " saved!");
	}
