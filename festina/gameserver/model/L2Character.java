...
import com.festina.gameserver.MapRegionTable.TeleportWhereType;
...
import com.festina.gameserver.model.actor.appearance.CharAppearance;
...
import com.festina.gameserver.model.actor.instance.L2PcInstance.SkillDat;
...
import com.festina.gameserver.model.actor.knownlist.CharKnownList;
import com.festina.gameserver.model.actor.knownlist.ObjectKnownList.KnownListAsynchronousUpdateTask;
import com.festina.gameserver.model.actor.stat.CharStat;
import com.festina.gameserver.model.actor.status.CharStatus;
...
import com.festina.gameserver.serverpackets.Attack;
import com.festina.gameserver.serverpackets.ChangeMoveType;
import com.festina.gameserver.serverpackets.ChangeWaitType;
import com.festina.gameserver.serverpackets.ExOlympiadSpelledInfo;
import com.festina.gameserver.serverpackets.MagicEffectIcons;
import com.festina.gameserver.serverpackets.MagicSkillCanceld;
import com.festina.gameserver.serverpackets.MagicSkillLaunched;
...
import com.festina.gameserver.serverpackets.PartySpelled;
...
import com.festina.gameserver.serverpackets.TargetUnselected;
import com.festina.gameserver.serverpackets.TeleportToLocation;
import com.festina.gameserver.skills.Calculator;
...
public abstract class L2Character extends L2Object
{
	...
	private static boolean _IsPsychicalMuted;
	  /** Zone system */
	   ...
	   public static final int ZONE_GRANDBOSS = 1024;
	...
	{ 
		teleToLocation(MapRegionTable.getInstance().getTeleToLocation(this, teleportWhere)); 
	}
	...
	public void doCast(L2Skill skill)
	{
		...
		if ((magicId >= 4551) && (magicId <= 4554))
			    {
			      L2PcInstance pl = (L2PcInstance)target;
			      for (L2Effect eff : pl.getAllEffects())
			      {
			        if (eff.getSkill().getId() == skill.getId())
			        {
			          int levels = eff.getLevel();
			          if ((levels > 0) && (levels < 10))
			          {
			            SkillTable.getInstance().getInfo(magicId, levels + 1).getEffects(this, target);
			            return;
			          }
			        }
			      }
	...
	public final void addEffect(L2Effect newEffect)
	{
		...
		synchronized(_effects) 
		{
			...
			if (getBuffCount() > Config.BUFFLIMIT && !doesStack(tempskill) && ((
	 ...
	 *
	 */
	private List<Integer> effectQueueInsert(int id, float stackOrder, List<Integer> stackQueue)
	{
...
//	private int _floatingRunSpeed;
	...
	public static final String ZONE_NOSUMMONFRIEND = null;
...
//		if (_move._heading < heading - 5 || _move._heading > heading  5)
     ...
     * 
     * @see com.festina.gameserver.model.L2Character.isInsideRadius(int x, int y, int z, int radius, boolean checkZ, boolean strictCheck)
     */
...
//	
	...
	{
        if (isInsidePeaceZone(player) && (!(player.getTarget() instanceof L2GuardInstance)))
		{
...
/*	public boolean isInsidePeaceZone(L2Object attacker, L2Object target)
	...
	public void doRebuff(boolean self)
	{
		...
		if (effects.isEmpty()) {
			sendPacket(new SystemMessage(SystemMessage.SKILL_NOT_AVAILABLE));
			return;
		}
		stopAllEffects();
...
//		return _effectList.getAllEffectsTable();
		...
		// If no effect found, return EMPTY_EFFECTS
		if (effects.isEmpty()) return EMPTY_EFFECTS_TABLE;
		// Return all effects in progress in a table
		int ArraySize = _effects.size();
        for (int i=0; i<ArraySize; i++) {
			if (effects.get(i) == null) break;
			if (effects.get(i).getSkill().getSkillType() != L2Skill.SkillType.BUFF){
			...
			for (L2Object target : targets)
			{
				if (target instanceof L2Character)
				{
					...
					if (activeChar != null)
					{
						if (skill.isOffensive())
						{
							if (player instanceof L2PcInstance && activeChar.checkIfPvP(player)) 
                            {
...
//	
	...
	public final void stopPsychicalMuted()
	{
		L2Effect effect = getEffect(L2Effect.EffectType.PSYCHICAL_MUTE);
		if (effect == null)
