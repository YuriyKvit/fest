...
import com.festina.gameserver.ItemsAutoDestroy;
import com.festina.gameserver.EventDroplist.DateDrop;
...
import com.festina.gameserver.ai.L2SiegeGuardAI;
...
import com.festina.gameserver.model.actor.instance.L2MinionInstance;
...
import com.festina.gameserver.model.actor.knownlist.AttackableKnownList;
import com.festina.gameserver.model.base.SoulCrystal;
        ...
        try 
        {
            if (killer instanceof L2PcInstance || killer instanceof L2SummonInstance) 
            {
                ...
                if (player.getParty() != null)
                {
                	...
                	for (List<?> list : tempMap.values())
                	{
                		Random rnd = new Random();
                		questList.add((QuestState)list.toArray()[rnd.nextInt(list.size())]);
                	}
                }
                else if (player.getQuestsForKills(this) != null) {
        	...
        	for (FastMap.Entry<L2Character, RewardInfo> entry = rewards.head(), end = rewards.tail(); (entry = entry.getNext()) != end;)
            {
                ...
                if (attackerParty == null)
                {
                    ...
                    if (attacker.getKnownList().knowsObject(this))
                    {
                        ...
                        //TODO: Сильно сложно... проще проще проше...
                        L2PcInstance cha = (L2PcInstance)attacker;
                        cha.addExpAndSp((int)attacker.calcStat(Stats.EXPSP_RATE, exp, null, null), 
                ...
                else
                {
                    ...
                    for(L2PcInstance pl : groupMembers)
                    {
                        ...
                        if(reward2 != null)
                        {
                            if(Util.checkIfInRange(1150, this, pl, true))
                            {
                        ...
                        if(summon != null && summon instanceof L2PetInstance)
                        {
                            ...
                            {
                                if(Util.checkIfInRange(1150, this, summon, true))
                                {
                    ...
                    if(partyDmg > 0)
                    {
                        attackerParty.distributeXpAndSp(exp, sp, rewardedMembers, attacker);
                    }
                ...
                if (((L2PcInstance)target).isInParty() && skill != null && skill.getTargetType() == L2Skill.SkillTargetType.TARGET_PARTY)
                {
                    ...
                    for (L2PcInstance member : members)
                    {
                        if (member == actor)
                        {
                            ...
                            if (Config.DEBUG)
                            	 _log.info("Mob detect party cast.\n");
                        }
                    }
     ...
     */
     private RewardItem calculateRewardItem(L2PcInstance lastAttacker, L2DropData drop, int levelModifier)
     {
         ...
         if (drop.getItemId() == 57) dropChance *= Config.RATE_DROP_ADENA;
        	 ...
        	 dropChance *= Config.RATE_DROP_ITEMS_GRAND_BOSS;
         else if (this instanceof L2RaidBossInstance)
        	 dropChance *= Config.RATE_DROP_ITEMS_RAID_BOSS;
         else 
      ...
      */
      private RewardItem calculateCategorizedRewardItem(L2PcInstance lastAttacker, List<L2DropData> categoryDrops, int levelModifier)
      {
          ...
          if ((this instanceof L2GrandBossInstance))
        	  categoryDropChance = (int)(categoryDropChance * Config.RATE_DROP_ITEMS_GRAND_BOSS);
          else if ((this instanceof L2RaidBossInstance))
        	  categoryDropChance = (int)(categoryDropChance * Config.RATE_DROP_ITEMS_RAID_BOSS);
          else {
         ...
         if (!isSeeded())
         {
           ...
           if (item != null)
           {
                 ...
                 if (Config.AUTO_LOOT) player.doAutoLoot(this, item); // Give this or these Item(s) to the L2PcInstance that has killed the L2Attackable
                     ...
                     if (Config.AUTO_LOOT_RAID) player.doAutoLoot(this, item); // Give this or these Item(s) to the L2PcInstance that has killed the L2Attackable
                     else DropItem(player, item); // drop the item on the ground
           }
